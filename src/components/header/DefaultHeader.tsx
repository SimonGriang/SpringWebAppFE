...
export const DefaultHeader = () => {
  ...
  if (!theme?.colors?.primary?.[6]) return null;
  const bgColor   = theme.colors.primary[6];
  const textColor = getContrastColor(bgColor);
  const initials  = `${user?.employeeFirstname?.[0] ?? "V"}${user?.employeeSurname?.[0] ?? "N"}`;

  return (
    <>
      {/*
        Kein pb={120} mehr — das war der Grund warum der Header-Container
        120px in den Content-Bereich hineinragte und das Panel scheinbar
        "in den Header" zeigte. Der Drawer ist ein Portal und braucht
        keinen Platzhalter im DOM.
      */}
      <header className={classes.header} style={{ height: "100%" }}>
        <Group justify="space-between" h="100%">
          ...
          {/* Logo */}
          {branding?.logoSrc ? (
            <img src={branding.logoSrc} alt="Logo" style={{ height: 40 }} />
          ) : (
          ...
          <Group h="100%" gap={0} visibleFrom="md">
            ...
            <a href="#" className={classes.link} style={{ color: textColor }}>Benutzerverwaltung</a>
            <a href="#" className={classes.link} style={{ color: textColor }}>Wunsch?</a>
          </Group>
          ...
          <Group gap="sm" visibleFrom="xs">
            ...
            >
              ...
              <Menu.Dropdown>
                <Menu.Item leftSection={<IconHeart size={16} color={theme.colors.red[6]} stroke={1.5} />}>Liked posts</Menu.Item>
                <Menu.Item leftSection={<IconStar size={16} color={theme.colors.yellow[6]} stroke={1.5} />}>Saved posts</Menu.Item>
                <Menu.Item leftSection={<IconMessage size={16} color={theme.colors.blue[6]} stroke={1.5} />}>Your comments</Menu.Item>
                ...
                <Menu.Item color="red" leftSection={<IconTrash size={16} stroke={1.5} />}>Delete account</Menu.Item>
              </Menu.Dropdown>
            </Menu>
          </Group>
      ...
      {/* Drawer ist ein Portal — braucht keinen Platzhalter im Layout */}
      <Drawer
        opened={drawerOpened}
        onClose={closeDrawer}
        size="100%"
      ...
      >
        <ScrollArea h="calc(100vh - 80px)" mx="-md">
          <Divider my="sm" />
          <a href="#" className={classes.drawerLink}>Home</a>
          <UnstyledButton className={classes.drawerLink} onClick={toggleLinks} fz="sm" p="md">
            <Center inline>
              <Box component="span" mr={5}>Zeiterfassung</Box>
              <IconChevronDown size={16} />
            </Center>
          </UnstyledButton>
          <Collapse in={linksOpened}>{drawerLinks}</Collapse>
          <a href="#" className={classes.drawerLink}>Benutzerverwaltung</a>
          <a href="#" className={classes.drawerLink}>Wunsch?</a>
          <Divider my="sm" />
          <UnstyledButton className={classes.drawerLink} onClick={toggleUserMenu} fz="sm" px="md">
            <Center inline>
              <Avatar radius="xl" size={32} styles={{ root: { backgroundColor: "var(--mantine-color-primary-6)" }, placeholder: { color: "white" } }}>
                {initials.toUpperCase()}
              </Avatar>
